...
import { go, cur } from '../core/router.js?v=20260909082613';
...
export async function panelPasangPin(anchor) {
  const { openPop } = await import('./menus/pop.js?v=20260909082613');
  const n = state.notes.find(x => x.id === state.openId);
  if (!n) return;
...
async function pasangDariPanel() {
  const { closeAll } = await import('./menus/pop.js?v=20260909082613');
