...
import { go, cur } from '../core/router.js?v=20260909063332';
...
export const layarKunciView = () => {
  ...
  if (!n)
    return `<div class="page"><div class="empty"><h3>Catatan tidak ada</h3>
      <p>Mungkin sudah dihapus.</p>
      <button type="button" class="btn btn-sec" data-go="notes">Ke daftar catatan</button></div></div>`;
...
export async function panelPasangPin(anchor) {
  const { openPop } = await import('./menus/pop.js?v=20260909063332');
  const n = state.notes.find(x => x.id === state.openId);
  if (!n) return;
...
async function pasangDariPanel() {
  const { closeAll } = await import('./menus/pop.js?v=20260909063332');
