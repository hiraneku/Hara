...
import { registerViews, onBeforeLeave, onAfterRender, cur, go } from '../core/router.js?v=20260918132843';
import { homeView, notesView } from './views/list.js?v=20260918132843';
import { editorView } from './views/editor.js?v=20260918132843';
import { miscViews, renderHasilCari, daftarTagHtml, setSaringTag, muatMutuGambar,
         setKueriCari } from './views/misc.js?v=20260918132843';
import { tugasView, reminderView } from './views/tugas.js?v=20260918132843';
import { bindEditor } from './editor/events.js?v=20260918132843';
import { renderBar }  from './bar/render.js?v=20260918132843';
import { bindPop, closeAll, openPop } from './menus/pop.js?v=20260918132843';
import { saveNow, updateCount, syncBtns, bacaEditor, tulisKeCatatan, saveSoon }
  from './editor/cleanup.js?v=20260918132843';
import { konfigurasi, onStatus, flush, reset as resetAutosave, STATUS, cobaUlang,
         adaPerubahanTertunda }
  from '../core/autosave.js?v=20260918132843';
import { bacaDraf, hapusDraf } from '../core/recovery.js?v=20260918132843';
import { toast } from '../core/toast.js?v=20260918132843';
import { esc } from '../core/dom.js?v=20260918132843';
import { blocksToDom, domToBlocks, touch } from './note-model.js?v=20260918132843';
import { renumber, pastikanKolomAkhir } from './editor/blocks.js?v=20260918132843';
import { pending, sticky, mati } from './editor/marks.js?v=20260918132843';
import { docEl, caretEnd } from './editor/caret.js?v=20260918132843';
import { resetHistory } from './editor/history.js?v=20260918132843';
import { pasangGambar, hapusGambar, bersihkanBlobYatim, pasangThumbDaftar } from './editor/image.js?v=20260918132843';
import { bebaskanUrl, pakaiRuang, ukuranTerbaca } from '../core/blobs.js?v=20260918132843';
import { BISA_SEMBUNYI, prefs, tersembunyi, toggleTampil, setGetar } from './bar/prefs.js?v=20260918132843';
import { renderBar as gambarBar } from './bar/render.js?v=20260918132843';
import { state, sinkronWelcome } from '../core/store.js?v=20260918132843';
import { labelMode, toggle as toggleTema, setAk } from '../core/theme.js?v=20260918132843';
import { purgeSampahOtomatis, pulihkanSampah, hapusPermanen, buatNoteBerjudul, openNote,
  ...
  from './model.js?v=20260918132843';
import { toggleModeBaca, sinkronModeBaca, modeBacaBerlaku } from './mode-baca.js?v=20260918132843';
import { bindBacaPlus, terapkanUkuranLayar, tutupCari, matikanZen }
  from './baca-plus.js?v=20260918132843';
import { bindDaftarIsi } from './daftar-isi.js?v=20260918132843';
import { setUrut, menuUrut, namaUrut } from './urut.js?v=20260918132843';
import { bukaJurnalHari } from './harian.js?v=20260918132843';
import { layarKunciView, bindKunci } from './kunci.js?v=20260918132843';
import { bindSwipe } from './swipe.js?v=20260918132843';
import { bindPilih } from './pilih.js?v=20260918132843';
import { bindBilahTeks } from './editor/bilah-teks.js?v=20260918132843';
import { bindTagWarna } from './tagwarna.js?v=20260918132843';
import { bindGaleri } from './galeri.js?v=20260918132843';
import { menuTemplat, terapkanTemplat, hapusTemplat } from './templat.js?v=20260918132843';
import { pasangSeret } from './drag.js?v=20260918132843';
import { muatPanels, tautkanSebutan } from './panels.js?v=20260918132843';
import { bindTataGambar, bersihkanPilihanGambar } from './tata-gambar.js?v=20260918132843';
import { setTag, stt } from './views/data.js?v=20260918132843';
import { simpanRiwayat, hapusRiwayat } from './cari.js?v=20260918132843';
import { aturMutu, opsiMutu } from './mutu-gambar.js?v=20260918132843';
import { aturProp, hapusProp, namaProp, barisProps, KET_PROP, contohProp } from './meta-ui.js?v=20260918132843';
import { cariJudul, judulSpan, tandaiTautan } from './wikilink.js?v=20260918132843';
import { t as tr, setBahasa, terjemahStatis } from '../core/i18n.js?v=20260918132843';
import { cadanganJson, eksporSemuaMarkdown, markdownDariCatatan, namaBerkasAman,
         unduh, buatZip, siapImpor, terapkanImpor }
  from './data-io.js?v=20260918132843';
...
export const notesModule = {
  ...
  init() {
    ...
    bindPilih();              /* pilih banyak: tahan-lama di baris daftar */
    bindBilahTeks();          /* bilah format di atas teks yang disorot */
    ...
    document.addEventListener('click', e => {
      ...
      const mu = e.target.closest('[data-mutu]');
      if (mu) {
        aturMutu(mu.dataset.mutu);
        muatMutuGambar();
        toast(tr('Mutu gambar') + ': ' + tr(opsiMutu().nama));
        return;
      }
    ...
    document.addEventListener('click', e => {
      ...
      /* ── tag: chip daftar, baris halaman tag, atau #tag di editor ──
         Klik #tag di dalam editor yang sedang bisa disunting (mode
         tulis): klik biasa = menaruh kursor/menyunting seperti teks
         biasa (sama seperti tautan [[…]]); pakai tombol pengubah
         (Ctrl/⌘/Alt) atau buka dalam mode baca untuk memfilter daftar.
         Di mode baca & di luar editor, satu ketukan langsung memfilter. */
      const tg = e.target.closest('[data-tag], .ed-doc .tg');
      if (tg) {
        const tagEditor = !!e.target.closest('.ed-doc .tg');
        const edKelas = document.querySelector('.ed');
        const modeBaca = edKelas && edKelas.classList.contains('baca');
        if (tagEditor && !modeBaca && !(e.ctrlKey || e.metaKey || e.altKey)) return;
        e.preventDefault();
        e.stopImmediatePropagation();
...
function segarkanSetelahSapuan() {
  ...
  if (w) w.scrollTop = sblm;
    ...
    /* riwayat pencarian (ronde 7): chip riwayat & tombol bersihkan */
    document.addEventListener('click', e => {
      const rx = e.target.closest('[data-cari-riwayat]');
      if (rx) {
        const ci = document.getElementById('cari-in');
        if (ci) ci.value = rx.dataset.cariRiwayat || '';
        simpanRiwayat(rx.dataset.cariRiwayat || '');
        renderHasilCari(rx.dataset.cariRiwayat || '');
        return;
      }
      if (e.target.closest('[data-riwayat-hapus]')) {
        hapusRiwayat();
        renderHasilCari('');
        return;
      }
    ...
    document.addEventListener('input', e => {
      ...
      if (cur === 'tags' && e.target && e.target.id === 'cari-tag') {
        setSaringTag(e.target.value);
        const w = document.getElementById('tags-daftar');
        if (w) w.innerHTML = daftarTagHtml(e.target.value);
      }
    });

    /* simpan kueri ke riwayat saat pengguna menekan Enter atau
       meninggalkan kolom cari — bukan tiap ketikan (riwayat tidak
       dipenuhi potongan kata yang belum selesai) */
    document.addEventListener('keydown', e => {
      if (e.key === 'Enter' && e.target && e.target.id === 'cari-in') {
        simpanRiwayat(e.target.value);
        renderHasilCari(e.target.value);
      }
    });
    document.addEventListener('change', e => {
      if (e.target && e.target.id === 'cari-in') simpanRiwayat(e.target.value);
    });
    ...
    onAfterRender(() => {
      ...
      muatPanels();
      muatMutuGambar();        /* C17: tanda pilihan mutu gambar */
      sinkronModeBaca();
      terapkanUkuranLayar();   /* A−/A+ tersimpan langsung berlaku */
      if (cur === 'search') {
        const ci = document.getElementById('cari-in');
        /* kueri terakhir digambar ulang — pindah layar / aksi dari hasil
           tidak menghapus pencarian yang sedang berjalan */
        if (ci && ci.value) renderHasilCari(ci.value);
        if (ci) ci.focus();
      }
    });
