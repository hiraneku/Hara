...
import { WARNA_TAG, tandaUntukCatatan, TANDA_TAG } from './label.js?v=20260918132648';
import { tagUntukTampil } from './tags.js?v=20260918132648';
import { openPop, closeAll } from './menus/pop.js?v=20260918132648';
import { cur, go } from '../core/router.js?v=20260918132648';
import { saveCatatanBuka } from './editor/cleanup.js?v=20260918132648';
...
export function panelTag(anchor) {
  ...
  if (!daftar.length) {
    toast(tr('Catatan belum punya tag — ketik #nama di isi'));
    return;
  }
  tagAktif = '';
  openPop(`<div class="pop-h">${tr('Warna tag catatan')}</div>
    <p class="pop-note">${tr('Ketuk tag untuk memilih warnanya.')}</p>
    ${daftar.map(t =>
...
export function bindTagWarna() {
  document.addEventListener('click', e => {
    ...
    if (sw) {
      ...
      toast(tr('Tag #{nama} diwarnai', { nama }));
      return;
    }
    ...
    if (hap) {
      ...
      closeAll();
      go(cur);
      toast(tr('Warna tag #{nama} dilepas', { nama }));
    }
  });
}
